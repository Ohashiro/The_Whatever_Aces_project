import { dynamicCountryList } from "./dynamicCountryList.js";
Promise.all([
...
function setCountries(value) {
  ...
  for (let i = 0; i < myCountries.length; ++i) {
    ...
    if (
      ...
      selectedCountries.includes(selectedCountry.value)
    ) {
      selectedCountry.classList.remove("selected");
...
export function fillRadarSelectSelector(countries) {
  const selected = document.querySelector(
    'input[name="comparisonRadar"]:checked'
  ).value;
  var radarCountrySelector = document.getElementsByClassName(
    "radarCountrySelector"
  )[0];
  if (selected == "countryradar") {
    var divElement = document.createElement("div");
    divElement.innerHTML = "Compare you country with another country:";
    var select = document.createElement("select");
    select.setAttribute("id", "radarCountryItems");
    var options = countries;
    options = ["Not selected"].concat(countries);
    ...
    for (var i = 0; i < options.length; i++) {
      ...
      radarCountrySelector.replaceChildren(divElement);
      radarCountrySelector.appendChild(select);
    }
  } else {
    radarCountrySelector.innerHTML = "";
  }
}
